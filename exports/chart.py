...
def generate_chart(invoices: list[dict], group_by: str = "Vendor", filename: str = "invoices_chart.png") -> str:
    """Aggregates Total Amount Due by `group_by` ("Vendor" | "Category" |
    "Month") and saves a bar chart PNG. Returns the file path."""
    key_func = _GROUP_KEY_FUNCS.get(group_by, _GROUP_KEY_FUNCS["Vendor"])
    ...
    ax.set_title(f"Total Amount Due by {group_by}", color=_TEXT_COLOR, fontsize=14, fontweight="bold")
